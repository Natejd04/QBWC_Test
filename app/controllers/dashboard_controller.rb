class DashboardController < ApplicationController
	...
	def index 
		...
		@orders = Order.where(c_invoiced: nil).where.not(:c_total => 0).where.not(:c_class => nil).where.not(:c_name => "Nate2 Davis").order(:c_class => "ASC")
	...
	# end channel calc		
		...
		@open_orders_count = Order.where(c_invoiced: nil).where.not(:c_total => 0).where.not(:c_class => nil).where.not(:c_name => "Nate2 Davis").count
