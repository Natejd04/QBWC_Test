class DashboardController < ApplicationController
	...
	def index 
		...
		@orders = Order.where(c_invoiced: nil).where.not(c_total: 0).order(:c_date => "DESC")
	...
	# end channel calc		
		...
		@journal_debit = Journal.joins(:account_line_items).where(:txn_date => Time.now.beginning_of_month..Time.now.end_of_month).where(["account_line_items.account_type = ? and account_line_items.account_id = ?", "debit", "143"]).sum('account_line_items.amount')
		@inv_gross_total = Invoice.joins(:items).where(:c_date => Time.now.beginning_of_month..Time.now.end_of_month).where("items.account_id = 143").sum("line_items.homecurrency_amount")
		@sr_gross_total = SalesReceipt.joins(:items).where(:txn_date => Time.now.beginning_of_month..Time.now.end_of_month).where("items.account_id = 143").sum("line_items.homecurrency_amount")
		...
		@pm_journal_debit = Journal.joins(:account_line_items).where(:txn_date => 1.month.ago.beginning_of_month..1.month.ago.end_of_month).where(["account_line_items.account_type = ? and account_line_items.account_id = ?", "debit", "143"]).sum('account_line_items.amount')
		@pm_inv_gross_total = Invoice.joins(:items).where(:c_date => 1.month.ago.beginning_of_month..1.month.ago.end_of_month).where("items.account_id = 143").sum("line_items.homecurrency_amount")
		@pm_sr_gross_total = SalesReceipt.joins(:items).where(:txn_date => 1.month.ago.beginning_of_month..1.month.ago.end_of_month).where("items.account_id = 143").sum("line_items.homecurrency_amount")
		@prior_m_total = ((@pm_inv_gross_total + @pm_sr_gross_total) - @pm_journal_debit)
			@vs = ((@month_total - @prior_m_total) / @month_total) * 100
		@open_orders_count = Order.where(c_invoiced: nil, c_closed: nil).count
